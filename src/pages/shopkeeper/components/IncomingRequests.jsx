    ...
    if (requests.length === 0) {
        return (
            <div className="bg-white rounded-[12px] border border-gray-200 p-10 text-center text-gray-500">
                No pending consignment requests.
            </div>
        );
    }
                ...
                <article key={request.id} className="bg-white rounded-[12px] border border-gray-200 overflow-hidden shadow-sm">
                    <div className="grid sm:grid-cols-[160px_1fr]">
                        ...
                        <div className="p-5">
                            ...
                            <h3 className="text-lg font-bold text-gray-900">{request.product?.name || "Unknown Product"}</h3>
                            <p className="text-sm text-gray-500 mb-4">{request.product?.category || "Uncategorized"}</p>
